...
void runKernel(id<MTLDevice> device, id<MTLCommandQueue> queue, id<MTLComputePipelineState> pipeline, NSArray* buffers, uint N) {
...
void apply_rope(std::vector<float>& vec, int pos, int head_dim,
    ...
    for (int step = 0; step < (int)input_ids.size() + max_new_tokens; step++) {
        @autoreleasepool {
            int token_id = (step < (int)input_ids.size())
                            ? input_ids[step]
                            : generated_ids.back();
            id<MTLBuffer> hidden = [device newBufferWithLength:HIDDEN_DIM * sizeof(uint16_t) options:MTLResourceStorageModeShared];
            {
            ...
            for (int l = 0; l < NUM_LAYERS; l++){
                uint d = HIDDEN_DIM;
                id<MTLBuffer> d_buf = [device newBufferWithBytes:&d length:sizeof(uint) options:MTLResourceStorageModeShared];
                
                id<MTLBuffer> normed = [device newBufferWithLength:HIDDEN_DIM * sizeof(uint16_t) options:MTLResourceStorageModeShared];
                runKernel(device, queue, pipeRmsNorm, @[hidden, layers[l].input_norm, normed, d_buf], HIDDEN_DIM);
                
                id<MTLBuffer> q_buf = matvec(device, queue, pipeMatvec, layers[l].q_proj, normed, NUM_HEADS * HEAD_DIM, HIDDEN_DIM);
                id<MTLBuffer> k_buf = matvec(device, queue, pipeMatvec, layers[l].k_proj, normed, NUM_KV_HEADS * HEAD_DIM, HIDDEN_DIM);
                id<MTLBuffer> v_buf = matvec(device, queue, pipeMatvec, layers[l].v_proj, normed, NUM_KV_HEADS * HEAD_DIM, HIDDEN_DIM);
                ...
                auto k_half = toHalfVec(k_f);
                auto v_half = toHalfVec(v_f);
                ...
                id<MTLBuffer> attn_buf = [device newBufferWithLength:NUM_HEADS*HEAD_DIM*sizeof(uint16_t) options:MTLResourceStorageModeShared];
                {
                ...
                id<MTLBuffer> attn_proj = matvec(device, queue, pipeMatvec, layers[l].o_proj, attn_buf, HIDDEN_DIM, NUM_HEADS * HEAD_DIM);
                runKernel(device, queue, pipeResAdd, @[hidden, attn_proj], HIDDEN_DIM);
                
                id<MTLBuffer> normed2 = [device newBufferWithLength:HIDDEN_DIM * sizeof(uint16_t) options:MTLResourceStorageModeShared];
                runKernel(device, queue, pipeRmsNorm, @[hidden, layers[l].post_norm, normed2, d_buf], HIDDEN_DIM);
                ...
                const uint16_t* gate_ptr = (const uint16_t*)[layers[l].gate contents];
                const uint16_t* up_ptr = (const uint16_t*)[layers[l].up contents];
                const uint16_t* down_ptr = (const uint16_t*)[layers[l].down contents];
                
                run_gate_up_batched(gate_ptr, up_ptr, x_mlp, gate_out, up_out, 1, INTERMEDIATE, HIDDEN_DIM);
                
                for (int j = 0; j < INTERMEDIATE; j++) {
                    float g = gate_out[j];
                    float silu = g / (1.0f + expf(-g));
                    mlp_mid[j] = silu * up_out[j];
                }
                
                run_down_batched(down_ptr, mlp_mid, down_out, 1, HIDDEN_DIM, INTERMEDIATE);
                
                id<MTLBuffer> mlp_buf = [device newBufferWithLength:HIDDEN_DIM * sizeof(uint16_t) options:MTLResourceStorageModeShared];
            ...
            id<MTLBuffer> final_hidden = [device newBufferWithLength:HIDDEN_DIM*sizeof(uint16_t) options:MTLResourceStorageModeShared];
            {
                uint d2 = HIDDEN_DIM;
                id<MTLBuffer> d2_buf = [device newBufferWithBytes:&d2 length:sizeof(uint) options:MTLResourceStorageModeShared];
                runKernel(device, queue, pipeRmsNorm, @[hidden, final_norm, final_hidden, d2_buf], HIDDEN_DIM);
            }
            
            id<MTLBuffer> logits_buf = matvec(device, queue, pipeMatvec, lm_head, final_hidden, VOCAB_SIZE, HIDDEN_DIM);
            ...
            if (step >= (int)input_ids.size()) generated_ids.push_back(next_token);
            else generated_ids.push_back(token_id);
            
            std::cout << "Token " << step << ": " << next_token << " (score " << max_val << ")\n";
        }
    }
    
    free(x_mlp); free(gate_out); free(up_out); free(mlp_mid); free(down_out);
